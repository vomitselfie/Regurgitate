...
For the short explanation and installation steps, start with the [README](../README.md).
This page covers optional commands, storage behavior, and advanced setup.
...
metadata—see [Architecture](architecture.md).

## Advanced installation

The standalone installer downloads a platform archive over HTTPS and verifies it
against `SHA256SUMS`. It installs the executable atomically under `~/.local/bin`
and pins the agent's hook and skill to that executable, so agent use does not
depend on `PATH`.

Installer options:

- `--agent codex` or `--agent claude`: install and connect the selected agent.
- `--agent none`: install only the executable (also the default).
- `--bin-dir <directory>`: choose another destination.
- `--version <version>`: choose a specific release instead of the latest.
- `--replace-skill`: explicitly replace differing skill content, after review.
...
| `regurgitate recall --query "csv importer" --brief --limit 2 --best-effort --token-budget 240` | Shows ranked lessons, or a compact non-blocking status |
...
object. The bundled skill has a tested 3,000-byte size limit. Its byte-based
estimate and the requested recall budget describe approximate context size, not exact
model tokenization. AoE setup adds an instruction containing its local
executable path. Whether the context pays for itself is what the
[paired benchmark](../benchmarks/README.md) is intended to measure.

These limits are not a promise about provider billing. The payoff comes when
a small recall prevents an agent from repeating a much larger
...
- [Decision briefs and relevance filtering](decision-briefs.md)
- [Roadmap](roadmap.md)
- [Source-format notes](source-format-notes.md)
- [Contributor and release guide](releasing.md)
- [Recall skill](../skills/regurgitate-recall/SKILL.md)
