...
version="${package_id##*@}"
archive="$dist_dir/praxis-v${version}-${platform}.tar.gz"
...
smoke_dir="$(mktemp -d "$repo_root/target/praxis-package-smoke.XXXXXX")"
cleanup() {
    rm -rf -- "$smoke_dir"
}
trap cleanup EXIT
...
for packaged_file in LICENSE README.md praxis; do
    if [[ ! -f "$smoke_dir/$packaged_file" ]]; then
        echo "release archive is missing $packaged_file" >&2
        exit 1
    fi
done
if [[ ! -x "$smoke_dir/praxis" ]]; then
    echo "packaged Praxis binary is not executable" >&2
    exit 1
fi

actual="$($smoke_dir/praxis --version)"
if [[ "$actual" != "praxis $version" ]]; then
