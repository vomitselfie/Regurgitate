        ...
        let mut stored = Vec::new();
        {
            let mut statement = self.connection.prepare(
                "SELECT id, created_at_ms, evidence_kind, schema_version, envelope_version, nonce, ciphertext
                 FROM events
                 WHERE project_token = ?1 AND evidence_kind = ?2
            ...
            let rows = statement.query_map(
                params![
                    token.as_slice(),
                    i64::from(evidence_kind.storage_code()),
                    i64::try_from(limit)?
                ],
...
mod tests {
    ...
    fn query_is_project_scoped_recent_first_and_bounded() {
        ...
        let events = store
            .recent_project_events(wanted, EvidenceKind::HookExecution, 1)
            .unwrap();
    ...
    #[test]
    fn hook_and_practice_windows_are_queried_independently() {
        let store = EncryptedStore::open_in_memory(&MasterKey::from_bytes([43; 32])).unwrap();
        let project_id = Uuid::from_u128(7);
        let hook = event(1, project_id, 200);
        let mut practice = event(2, project_id, 100);
        practice.session_id = None;
        practice.agent = None;
        practice.evidence_kind = EvidenceKind::LearnedPractice;
        practice.task = Some(TaskKind::DataImport);
        practice.strategy = Some(Strategy::PerSubjectStreaming);
        store.append(&hook).unwrap();
        store.append(&practice).unwrap();

        let hooks = store
            .recent_project_events(project_id, EvidenceKind::HookExecution, 10)
            .unwrap();
        let practices = store
            .recent_project_events(project_id, EvidenceKind::LearnedPractice, 10)
            .unwrap();

        assert_eq!(hooks, vec![hook]);
        assert_eq!(practices, vec![practice]);
    }
}
